#!/usr/bin/env python3
"""Download the newest camera movie into the newest measurement session folder."""
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Download the newest camera movie into the newest user-data session folder."
    )
    ...
    parser.add_argument(
        "--output-name",
        help="Destination filename. Defaults to the original camera filename.",
    )
    parser.add_argument(
        "--extensions",
    ...
    destination = unique_destination(session_dir / output_name, args.force)
    ...
    copy_gio_file(latest_movie.uri, destination, force=args.force)
    write_metadata(destination, session_dir, latest_movie)
    print(f"Downloaded: {destination}")
    print(f"Metadata: {destination.with_suffix(destination.suffix + '.json')}")
    return 0
