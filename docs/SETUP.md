...
9. For SR551 mode, connect SR551 output A to Moku Input 2 and output B to Moku Input 3. Use direct/1x coax or a 1x probe on the SR551 input; do not put a 10x passive probe between the shunt and SR551 input A, because that attenuates the small shunt signal before amplification. The SR551 output is always balanced differential, so the app records both outputs and computes `CH2 - CH3`. On the SR551 input side, use either single-ended mode with input A on shunt high and the selector at `A`, or differential mode with input A on shunt high, input B on shunt low/ground, and the selector at `A-B`. Use `moku_current_mode: "sr551_differential"`, `current_shunt_ohms: 330`, and `current_amplifier_gain: 10`.
